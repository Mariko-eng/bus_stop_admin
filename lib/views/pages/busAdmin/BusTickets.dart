...
class BusCompanyTickets extends StatefulWidget {
  ...
  const BusCompanyTickets({Key key,this.user}) : super(key: key);
    ...
    return DefaultTabController(
        ...
        child: Scaffold(
          ...
          body: Column(
            children: [
              FutureBuilder(
                future: getAllTripsNumbersForBusCompany(companyId: widget.user.uid),
              ...
              Expanded(
                child: TabBarView(
                    children: [
                      BusTicketsActive(user: widget.user,tripId: _tripIdController.text,),
                      BusTicketNonActive(user: widget.user,tripId: _tripIdController.text)
                    ]),
              )
            ],
                            ...
                            child: Row(
                              ...
                              children: [
                                Expanded(
                                    child: TextField(
                                      decoration: const InputDecoration(
                                          hintText: "Search Here (Enter Trip Number)",
                                          hintStyle: TextStyle(
                                            fontSize: 12
                                          ),
                                        ...
                                        onTap: () {
                                          setState2(() {
                                            _tripNoController.text =
                                                sr[i].tripNumber;
                                            _tripIdController.text= sr[i].tripId;
                                          });
                                          Navigator.of(context).pop();
                                          setState(() {});
                                        },
                                      ...
                                      GestureDetector(
                                        onTap: () {
                                          setState2(() {
                                            _tripNoController.text =
                                                data[i].tripNumber;
                                            _tripIdController.text= data[i].tripId;
                                          });
                                          Navigator.of(context).pop();
                                          setState(() {});
                                        },
